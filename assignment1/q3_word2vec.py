...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    ### YOUR CODE HERE
    z = np.dot(outputVectors,predicted) # inputVector为嵌入矩阵(dim:n_vocabulary, embedding_size)
                            # outputVectors为上下文矩阵(dim:embedding_size, n_vocabulary)
    yhat = softmax(z)
    cost = -np.log(yhat[target])
    
    yhat_copy = yhat.copy()
    yhat_copy[target] -= 1.0
    grad = np.outer(yhat_copy, predicted)
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    ### YOUR CODE HERE
    x = tokens[currentWord] # 输入单词转化为下标
    predicted = inputVectors[x] # 得到词向量表示
    targets = [tokens[word] for word in contextWords] # 目标词转换为下标
